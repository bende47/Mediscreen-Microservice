...
public class ClientUIController {
	...
	@GetMapping(value = "rapport/stat/{idpatient}")
	public String rapport(@PathVariable("idpatient") String idpatient)  {		

		SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
		
		JSONObject jsonObject = new JSONObject();
		PatientBean patient = patienProxy.find(idpatient);
		...
		for (HistoryBean histBean : listHistoriquePatient) {			
			for (DeclencheursBean decl : listDecl) {
				 if(histBean.getNotes().toUpperCase().contains(decl.getLibelle().toUpperCase())) {
					 cpt++;
				 }
			}			
		}
		...
		jsonObject.put("Birthday", sdf.format(patient.getBirthday()));
		jsonObject.put("Sex", patient.getSex());
		jsonObject.put("Risque", risque);
